...
受 PAI (Path-Aware Injection) 论文启发，我们探索在 DDIM 采样过程中引入**频域路径偏转**，希望进一步增强水印的鲁棒性和抗篡改能力。

**核心思想**：
- 传统方案：水印仅嵌入在初始噪声中
- 路径偏转：在推理的前 N 步持续注入水印信号，使水印更深地“烙印”到生成轨迹中

**研究问题**：路径偏转在多密钥识别场景下是否同样有效？

### 7.2 技术方案

**通用偏转信号**：使用一个固定的、与供应商无关的偏转 pattern，所有供应商共用：
- 生成时：在前 N 步对 latent 进行频域偏转（加入偏转信号）
- 检测时：在 DDIM inversion 过程中进行逆向补偿（减去偏转信号）

```
生成过程：
z_T (带水印初始噪声) → [偏转] → z_{T-1} → [偏转] → ... → z_0 (最终图像)
                        ↑                    ↑
                    前 N 步加入偏转信号

检测过程：
图像 → VAE encode → [补偿] → DDIM inversion → [补偿] → 重建初始噪声 → 匹配密钥
                     ↑                          ↑
                 前 N 步减去偏转信号
```

**关键参数**：
- `deflection_steps`：偏转步数（默认 5）
- `deflection_strength`：偏转强度（默认 0.3）
...
|------|------|------|
| Baseline | 无偏转 | 对照组 |
| 实验 1 | steps=5, strength=0.3 | 默认参数 |
| 实验 2 | steps=5, strength=0.1/0.2/0.5 | 强度调优 |
| 实验 3 | steps=3/10/15, strength=0.3 | 步数调优 |

### 7.4 实验结果

**实验配置**：5 keys, 100 trials, 双频带编码 (R=3-14)

#### 与 Baseline 对比

| 配置 | Clean | Rot75 | JPEG25 | C&S75 | Blur8 | Noise | Bright | **平均** |
|------|-------|-------|--------|-------|-------|-------|--------|----------|
| **Baseline (无偏转)** | 100% | 100% | 100% | **90%** | 100% | 100% | 98% | **98.3%** |
| 最优偏转 (s=5, m=0.1) | 100% | 100% | 100% | 84% | 100% | 99% | 99% | 97.4% |

#### 强度调优 (steps=5)

| 配置 | Clean | Rot75 | JPEG25 | C&S75 | Blur8 | Noise | Bright | **平均** |
|------|-------|-------|--------|-------|-------|-------|--------|----------|
| **s=5, m=0.1** | 100% | 100% | 100% | **84%** | 100% | 99% | 99% | **97.4%** ✓ |
| s=5, m=0.2 | 100% | 100% | 100% | 80% | 100% | 99% | 100% | **97.0%** |
| s=5, m=0.3 | 100% | 100% | 100% | 80% | 99% | 99% | 99% | **96.7%** |
| s=5, m=0.5 | 100% | 100% | 100% | 81% | 100% | 99% | 99% | **97.0%** |

#### 步数调优 (strength=0.3)

| 配置 | Clean | Rot75 | JPEG25 | C&S75 | Blur8 | Noise | Bright | **平均** |
|------|-------|-------|--------|-------|-------|-------|--------|----------|
| s=3, m=0.3 | 100% | 100% | 100% | 81% | 100% | 99% | 99% | **97.0%** |
| s=5, m=0.3 | 100% | 100% | 100% | 80% | 99% | 99% | 99% | **96.7%** |
| s=10, m=0.3 | 100% | 100% | 100% | 81% | 100% | 99% | 100% | **97.1%** |
| s=15, m=0.3 | 100% | 100% | 100% | 79% | 100% | 99% | 99% | **96.7%** |

### 7.5 实验结论

#### 关键发现：抗篡改性 vs 可识别性的 Trade-off

| 指标 | Baseline (无偏转) | 最优偏转 (s=5, m=0.1) | 变化 |
|------|------------------|------------------------|------|
| 平均准确率 | **98.3%** | 97.4% | ↓ 0.9% |
| C&S 攻击 | **90%** | 84% | ↓ 6% |
| 抗篡改能力 | 一般 | 较强 | ↑ |
...
1. **偏转的设计目标**：增强抗篡改能力（让水印更难被移除）
2. **偏转的副作用**：检测时需要补偿，补偿误差会累积
3. **对 C&S 攻击的影响**：偏转 + 补偿后，频域结构更容易被裁剪缩放破坏

#### 结论

> **在多密钥识别场景下，路径偏转（抗篡改增强）与识别准确率存在 trade-off。**

- **偏转的价值**：增强抗篡改能力（适用于单密钥验证场景）
- **偏转的代价**：降低识别准确率，尤其是 C&S 攻击下
- **建议**：在 RingID 多密钥识别场景下，**保持初始噪声注入的简洁性比引入路径偏转更优**

这一发现与成员 A 在 Tree-Ring 单密钥验证场景下的结论形成互补：
- **单密钥验证**：偏转可能有益（增强抗篡改）
- **多密钥识别**：偏转无益（降低准确率）
...
# 密钥容量测试
python scripts/key_capacity_test.py --keys 5,10,20,30,50 --gpu 2

# 频段选择对比实验
python scripts/frequency_band_test.py --gpu 2
